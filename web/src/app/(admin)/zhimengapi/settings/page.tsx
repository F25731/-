                                ...
                                <Row gutter={16}>
                                        ...
                                        <Alert
                                            ...
                                            description={
                                                <ul style={{ margin: 0, paddingLeft: 20 }}>
                                                    <li>支持 OpenAI 兼容格式的中转站接口</li>
                                                    <li>模型地址在下方"私有配置"的渠道中配置</li>
                                                    <li>用户在前端为每个模型单独配置 API Key</li>
                                                    <li>视频模型配置后，用户可在画布节点和视频生成面板的提示词框右下角选择模型</li>
                                                </ul>
                                            }
                                            type="info"
                                            showIcon
                                    ...
                                    <Col xs={24} md={6}>
                                        <Form.Item name={["public", "modelChannel", "defaultModel"]} label="默认模型">
                                            <Select showSearch allowClear placeholder="选择默认模型" options={publicModels.map((item) => ({ label: item, value: item }))} />
                                        </Form.Item>
                                    </Col>
                                    <Col xs={24} md={6}>
                                        <Form.Item name={["public", "modelChannel", "defaultImageModel"]} label="默认图片模型">
                                            <Select showSearch allowClear placeholder="选择图片模型" options={publicModels.map((item) => ({ label: item, value: item }))} />
                                        </Form.Item>
                                    </Col>
                                    <Col xs={24} md={6}>
                                        <Form.Item name={["public", "modelChannel", "defaultVideoModel"]} label="默认视频模型" extra="用户可在画布节点选择">
                                            <Select showSearch allowClear placeholder="选择视频模型" options={publicModels.map((item) => ({ label: item, value: item }))} />
                                        </Form.Item>
                                    </Col>
                                    <Col xs={24} md={6}>
                                        <Form.Item name={["public", "modelChannel", "defaultTextModel"]} label="默认文本模型">
                                            <Select showSearch allowClear placeholder="选择文本模型" options={publicModels.map((item) => ({ label: item, value: item }))} />
                                        </Form.Item>
                                    </Col>
                                    <Col span={24}>
                        ...
                        <Form form={form} layout="vertical" initialValues={emptySettings} requiredMark={false}>
                            <Flex vertical gap={12}>
                                ...
                                <Alert
                                    ...
                                    description={
                                        <ul style={{ margin: 0, paddingLeft: 20 }}>
                                            <li><strong>协议:</strong> 目前支持 OpenAI 兼容格式</li>
                                            <li><strong>接口地址:</strong> 中转站的 base URL (如 https://api.example.com)</li>
                                            <li><strong>API Key:</strong> 后台管理的主密钥，用于拉取模型列表和测试</li>
                                            <li><strong>用户密钥:</strong> 前端用户在配置弹窗中为每个模型单独配置密钥</li>
                                        </ul>
                                    }
                                    type="info"
                                    showIcon
