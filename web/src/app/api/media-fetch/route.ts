...
const MAX_MEDIA_FETCH_BYTES = 250 * 1024 * 1024;
...
export async function GET(request: NextRequest) {
    ...
    if (contentLength > MAX_MEDIA_FETCH_BYTES) {
        return NextResponse.json({ code: 1, msg: "Media file is too large" }, { status: 413 });
    }

    const body = await readLimited(response, MAX_MEDIA_FETCH_BYTES);
    if (!body) return NextResponse.json({ code: 1, msg: "Media file is too large" }, { status: 413 });
