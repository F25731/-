...
import { Camera, Copy, Download, FolderPlus, Image as ImageIcon, Images, Info, Lock, LockOpen, Maximize2, MessageSquare, Minus, Pencil, Plus, RefreshCw, Scissors, Settings2, Trash2, Upload } from "lucide-react";
...
export function CanvasNodeHoverToolbar({
    ...
    return (
        ...
        >
            ...
            {hasImage ? <ToolbarAction title="上传参考图并连接到当前图片" label="参考改图" icon={<Images className="size-4" />} onClick={() => onUploadReference(node)} /> : null}
...
export function CanvasNodeInfoModal({ node, open, onClose }: { node: CanvasNodeData | null; open: boolean; onClose: () => void }) {
    ...
    const batchCount = node?.type === CanvasNodeType.Image && node.metadata?.isBatchRoot ? (node.metadata?.batchChildIds?.length || 0) + 1 : 0;
    const json = useMemo(() => {
        if (!node) return "";
    ...
    const displayPrompt = node?.metadata?.originalPrompt || node?.metadata?.prompt || "";
                                ...
                                <InfoRow
                                    ...
                                    value={
                                        <div className="space-y-2">
                                            <div className="select-text whitespace-pre-wrap break-words rounded-lg border px-3 py-2 leading-6" style={{ borderColor: theme.node.stroke, background: theme.node.fill }}>
                                                {displayPrompt}
                                            </div>
                                            <button
                                                type="button"
                                                className="inline-flex h-8 items-center gap-1.5 rounded-lg border px-2.5 text-xs transition hover:opacity-80"
                                                style={{ borderColor: theme.node.stroke, color: theme.node.text }}
                                                onMouseDown={(event) => event.stopPropagation()}
                                                onClick={(event) => {
                                                    event.stopPropagation();
                                                    copyText(displayPrompt, "提示词已复制");
                                                }}
                                            >
...
function InfoRow({ label, value }: { label: string; value: ReactNode }) {
    return (
        <div className="grid grid-cols-[72px_minmax(0,1fr)] gap-3">
            ...
            <span className="min-w-0 select-text whitespace-pre-wrap break-words">{value}</span>
        </div>
    );
}
